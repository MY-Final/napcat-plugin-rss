...
│         │                  │                  │                        │
│  ┌──────┴──────┐    ┌──────┴──────┐    ┌──────┴──────┐                 │
│  │  RSS Service │    │ Msg Service │    │   Puppeteer │                 │
│  │              │    │              │    │   Service   │                 │
│  │ Parser       │    │ Text         │    │             │                 │
│  │ Fetcher      │    │ Image        │    │   Client    │                 │
│  │ Storage      │    │ Forward      │    │   Templates │                 │
│  └──────┬───────┘    └──────┬───────┘    └──────┬──────┘                 │
│         │                  │                  │                        │
└─────────┼──────────────────┼──────────────────┼─────────────────────────┘
          │                  │                  │
...
| 模块 | 描述 |
|------|------|
| **Core** | 核心模块，负责定时调度和状态管理 |
| **RSS Service** | RSS 解析、获取、存储 |
| **Message Service** | 消息发送（文本/图片/合并转发） |
| **Puppeteer Service** | HTML 模板渲染为图片 |
| **Message Handler** | 处理群消息中的命令 |
| **WebUI** | 可视化管理界面 |
| **Config** | 配置文件管理 |
